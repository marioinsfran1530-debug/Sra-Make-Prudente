...
export default async function AdminPedidoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="text-xs text-cinza mb-4">{new Date(order.createdAt).toLocaleString("pt-BR")}</p>
      <div className="bg-white rounded-2xl p-4 mb-4" style={{ boxShadow: "0 2px 10px rgba(35,20,42,0.06)" }}>
        ...
        <a
          href={waLink(`Oi ${order.customerName}! Atualização do seu pedido #${order.number} na Sra Make Prudente:\n\nStatus atual: ${STATUS_LABEL[order.status] ?? order.status}\n\nQualquer dúvida, estamos à disposição.`, order.customerPhone)}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full text-white"
          style={{ backgroundColor: "#25D366" }}
        ><MessageCircle size={13} /> Abrir WhatsApp</a>
      </div>
      <div className="bg-white rounded-2xl p-4 mb-4" style={{ boxShadow: "0 2px 10px rgba(35,20,42,0.06)" }}>
        <p className="text-xs font-bold text-texto mb-2">Produtos</p>
        {order.items.map((item) => (
          <div key={item.id} className="flex justify-between text-sm mb-1">
            <span className="text-texto">{item.name}{item.variantName ? ` (${item.variantName})` : ""} × {item.qty}</span>
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className="flex justify-between text-sm">
      ...
      <span className={bold ? "font-extrabold" : "font-medium"} style={{ color: bold ? "#A6157A" : "#23142A" }}>{value}</span>
    </div>
  );
}
