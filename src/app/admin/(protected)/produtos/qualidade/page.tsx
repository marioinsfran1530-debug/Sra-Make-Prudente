...
function evaluateProduct(product: {
  ...
  return { issues, score: 100 - penalty, effectiveStock };
}
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-rosa-profundo">Qualidade de dados</p>
          ...
          <p className="mt-1 max-w-3xl text-sm text-cinza">
            Prioriza cadastros que podem prejudicar Google, Merchant Center, busca interna e conversão. O índice é operacional e não altera produtos automaticamente.
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/admin/analise" className="rounded-xl border border-rosa/20 bg-white px-4 py-2.5 text-xs font-bold text-rosa-profundo">Ver análise</Link>
          <Link href="/admin/produtos/novo" className="rounded-xl bg-rosa-profundo px-4 py-2.5 text-xs font-bold text-white">Novo produto</Link>
        </div>
      </div>
      ...
      <div className="mb-5 rounded-2xl border border-rosa/10 bg-white p-4 shadow-sm">
        <p className="text-sm font-bold text-texto">Padrão recomendado para novos cadastros</p>
        <div className="mt-3 grid gap-2 text-xs text-cinza md:grid-cols-2 xl:grid-cols-4">
          <p>• Nome claro com produto + característica + marca.</p>
          <p>• Marca real e EAN/GTIN quando existir.</p>
          <p>• Descrição útil com pelo menos 50 caracteres.</p>
          <p>• Foto principal limpa, preço e estoque conferidos.</p>
        </div>
      </div>
